...
fn fd_is_launchd_ipc(fd: i32, want: &str) -> bool {
    ...
    unsafe {
        ...
        if want_c.as_ptr().is_null() || libc::stat(want_c.as_ptr(), &mut want_st) != 0 {
            return false;
        }
...
async fn ipc_dispatch(
    ...
    let (ok, resp) = match (req.cmd.as_str(), authorized) {
        ...
        ("connect", true) => {
            ...
            if st.access_token.is_none() {
            ...
            } else if cfg.dests.is_empty() {
                svc::ipc_count("connect", false);
                (false, err_resp("no dests configured"))
            } else {
                ...
                let cli = aztna_client::Cli {
                    ...
                    cmd: aztna_client::Cmd::Access {
                        ...
                        serve: true,
                        gateway_relay: None,
                        transport: aztna_client::TransportKind::Auto,
                        bind: "127.0.0.1".into(),
                        print_token: false,
                        dns: cfg.dns.clone(),
                        metrics: None,
