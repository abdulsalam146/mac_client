...
// W43 S3: system-wide split-DNS core (config, normalization, journal, NRPT)
pub mod splitdns;
...
pub enum Cmd {
    ...
    Access {
        ...
        /// data transport to the gateway [P2-4 FR-NET-002]; W40: NO flag =
        /// follow the tenant transport mode (policy-version poll); an
        /// explicit value pins the carrier for this invocation (dev/test
        /// escape hatch — the service host exposes no override)
        #[arg(long, value_enum)]
    ...
    Bench {
        ...
        tunnels: u32,
        /// W40: no flag = the tenant transport mode applies here too
        #[arg(long, value_enum)]
        transport: Option<TransportKind>,
    ...
    DnsZones,
    /// W43 S3 [plan §11 rollback]: remove every aZTNA-owned OS resolver
    /// rule by hand (the escape hatch when auto mode misbehaves; needs an
    /// elevated context on Windows)
    SplitdnsClean,
...
pub enum TransportKind {
    ...
    TlsTcp,
    Auto,
    /// W40 [tenant transport mode]: the auto machinery with the carrier
    /// preference inverted — TLS-TCP first under the same probe budget,
    /// QUIC in the same dial sequence on the same fallback-eligibility
    /// predicate; plain TCP is never selected (both carriers are mTLS)
    TcpFirst,
}
...
pub struct ClientState {
    ...
    /// W40 [tenant transport mode]: last-known controller-desired mode,
    /// applied from EVERY policy-version poll response and persisted only
    /// on change. Tolerant read: a non-string/corrupt value loads as None
    /// (= auto) — an advisory field must never poison the state load.
    #[serde(default, deserialize_with = "de_transport_mode_lenient")]
...
/// Tolerant `transport_mode` state read [W40 fail-safe contract]: a
/// non-string value degrades to None (= auto) instead of failing the
/// WHOLE ClientState load (a corrupt advisory field is never a
/// logout-equivalent state failure).
fn de_transport_mode_lenient<'de, D>(d: D) -> std::result::Result<Option<String>, D::Error>
where
...
// ---- W40 [tenant transport mode] ---------------------------------------
// The controller-desired transport mode: applied from EVERY policy-version
// poll response (never gated on a version bump — a mode change bumps
// nothing in policy_version) and seeded at boot from the persisted
// last-known value. Resolution precedence (plan §3.5): explicit CLI
// --transport > this tenant mode > auto. The mode is a DESIRED carrier
// preference only — never an enforcement control, never a trust change
// (mTLS is intact in every mode).
static TENANT_MODE: std::sync::RwLock<aztna_common::TransportMode> =
    std::sync::RwLock::new(aztna_common::TransportMode::Auto);

pub fn tenant_mode() -> aztna_common::TransportMode {
    *TENANT_MODE.read().unwrap()
}

fn set_tenant_mode(mode: aztna_common::TransportMode) {
    *TENANT_MODE.write().unwrap() = mode;
}

/// The mode ACTUALLY APPLIED for a dial — the carrier-selected metric's
/// `mode` label (post-resolution, CLI override included). The legacy
/// plain-TCP CLI value gets its own label: no tenant mode produces it.
fn applied_mode_label(k: TransportKind) -> &'static str {
    match k {
        TransportKind::Tcp => "cli_legacy_tcp",
        TransportKind::Quic => "quic_only",
        TransportKind::TlsTcp => "tcp_only",
        TransportKind::Auto => "auto",
        TransportKind::TcpFirst => "tcp_first",
    }
}

/// W40 §3.5 resolution. The legacy `tcp` (plaintext) kind is reachable
/// ONLY via the explicit CLI value — no tenant mode maps to it.
fn resolve_transport(explicit: Option<TransportKind>) -> TransportKind {
    match explicit {
        Some(k) => k,
        None => match tenant_mode() {
            aztna_common::TransportMode::Auto => TransportKind::Auto,
            aztna_common::TransportMode::TcpFirst => TransportKind::TcpFirst,
            aztna_common::TransportMode::QuicOnly => TransportKind::Quic,
            aztna_common::TransportMode::TcpOnly => TransportKind::TlsTcp,
        },
    }
}

/// Apply a poll-received tenant mode: swap the dial-time resolution
/// source, log the effective-mode change, and persist the last-known
/// value (change-only) so a pre-first-poll boot starts where the poll
/// left off.
fn apply_tenant_mode(mode: aztna_common::TransportMode) {
    let prev = tenant_mode();
    if prev == mode {
        return;
    }
    log_event(
        "transport_mode_applied",
        &format!("{} -> {}", prev.as_str(), mode.as_str()),
    );
    set_tenant_mode(mode);
    if let Ok(mut st) = load_state() {
        if st.transport_mode.as_deref() != Some(mode.as_str()) {
            st.transport_mode = Some(mode.as_str().to_string());
            if let Err(e) = save_state(&st) {
                println!("[transport] persisting transport_mode failed: {e}");
            }
        }
    }
}
...
/// Always refreshes: a cached cert from a previous device must not
/// survive a reused state dir [F-06 multi-device runs]. (W37: re-enroll
/// on an already-enrolled state is refused by the enroll guard, so a
/// fresh enroll reaching here means a new/wiped state dir.)
...
/// `enroll` (writes once, keyed by the device_id it issues) and a running
/// serve's device_id cannot change; since W37 the enroll guard REFUSES an
/// already-enrolled state, so the CLI can no longer re-enroll over a live
/// device_id (the gap-register 2026-09-07 overwrite-under-running-process
/// caveat is closed at the source). The cache re-reads on any device_id
/// mismatch — belt-and-braces; a hypothetically stale key would fail LOUD
/// at the controller (posture_rejections_total{invalid_sig}), not
/// silently.
...
async fn posture_reporter(mclient: reqwest::Client, controller_url: String, device_id: String) {
    ...
    const FULL_REFRESH_TICKS: u64 = 10; // bound divergence: full push ≤ every 10th tick
                                        // W43 S2: engine-scoped — bind once (the token outlives the select arm)
    let pr_cancel = svc::engine_cancel();
    loop {
        ...
        match report {
            Ok(r) if !r.is_null() => {
                ...
                let changed = hash
                    ...
                    .unwrap_or(true);
                let force_full = tick.is_multiple_of(FULL_REFRESH_TICKS);
                let send_full = || {
        ...
        tick += 1;
        // W43 S2: engine-scoped — a disconnected session must not keep
        // posting with its dead token (401 spam until reconnect)
        tokio::select! {
            biased;
            _ = pr_cancel.cancelled() => return,
...
// ---------- W37 [FR-BND-004]: enroll already-enrolled guard ----------

/// Classification of an already-enrolled local state, for the enroll
/// guard [gap-register post-audit note 2026-09-07, closed by W37]: a
/// second enroll against a live state would overwrite
/// posture-keys/<device_id>.hex under a running process (invalid_sig
/// storm until restart), so enroll refuses fail-closed. Pure — no env,
/// no fs, no clock. Debug is required (the wiring formats `{es:?}`);
/// Clone/PartialEq/Eq keep the classifier unit tests assertion-clean.
...
enum EnrolledState {
    ...
    /// cert present without a device id — inconsistent/legacy state;
    /// refuse belt-and-braces rather than risk a key overwrite
    CertOnly,
}

/// Classification precedence (authoritative, docs/w37-enroll-guard-plan.md §4):
/// device_id + cert_pem → Approved · device_id only → Pending ·
/// cert_pem only → CertOnly · neither → None (enroll proceeds).
fn enrolled_state(st: &ClientState) -> Option<EnrolledState> {
    match (st.device_id.is_some(), st.cert_pem.is_some()) {
        (true, true) => Some(EnrolledState::Approved {
            device_id: st.device_id.clone().unwrap_or_default(),
        }),
        (true, false) => Some(EnrolledState::Pending {
            device_id: st.device_id.clone().unwrap_or_default(),
        }),
        (false, true) => Some(EnrolledState::CertOnly),
        (false, false) => None,
    }
}

/// State-specific operator message for a refused enroll. Pure — the
/// state dir is passed in (the arm supplies the real path; `state_path`
/// reads the env and stays out of the tested surface). All three are
/// framed as LOCAL state and make no claim about the device row's
/// liveness on the controller. `renew` is suggested only for Approved —
/// renewal presents the CURRENT cert, so it is not a pending-state
/// remedy.
fn enroll_refusal_msg(es: &EnrolledState, state_dir: &str) -> String {
    match es {
        EnrolledState::Pending { device_id } => format!(
            "already enrolled as {device_id} (pending approval - no certificate yet): \
             wait for an admin to approve, then run `login` (`status` to check). \
             To discard this enrollment, remove the state dir {state_dir} \
             and enroll with a fresh token"
        ),
        EnrolledState::Approved { device_id } => format!(
            "already enrolled as {device_id} (approved): run `renew` to re-issue \
             the device certificate on the same identity, or remove the state \
             dir {state_dir} to re-enroll as a new device"
        ),
        EnrolledState::CertOnly => format!(
            "inconsistent local state: a device certificate is present without \
             a device id (corrupt or legacy state dir). Remove the state dir \
             {state_dir} and enroll with a fresh token"
        ),
    }
}
...
pub async fn run_with(cli: Cli) -> Result<()> {
    ...
    // W40: seed the tenant mode from the persisted last-known value
    // (corrupt/absent -> auto, never a state-load failure); every poll
    // response re-applies the controller's current mode on top.
    if let Some(raw) = st.transport_mode.as_deref() {
        let (mode, bad) = aztna_common::TransportMode::parse_lenient(raw);
        if bad {
            println!(
                "[transport] persisted transport_mode '{raw}' unparseable - auto [W40 fail-safe]"
            );
            log_event(
                "transport_mode_parse_fallback",
                &format!("value={raw} (persisted) - auto"),
            );
        }
        if mode != aztna_common::TransportMode::Auto {
            log_event(
                "transport_mode_applied",
                &format!("auto -> {} (persisted)", mode.as_str()),
            );
        }
        set_tenant_mode(mode);
    }
    ...
    match cli.cmd {
        ...
        Cmd::Enroll { token, hostname } => {
            // W37 [FR-BND-004 guard]: refuse BEFORE key generation, any
            // enrollment-state write, or the POST — the enrollment token
            // is not consumed; the only intentional filesystem change on
            // refusal is the enroll_rejected audit event (client.log).
            // glmsvc is unaffected: its own device_id.is_none() bootstrap
            // gate precedes any call here.
            if let Some(es) = enrolled_state(&st) {
                log_event("enroll_rejected", &format!("{es:?}"));
                bail!(
                    "{}",
                    enroll_refusal_msg(&es, &state_path().parent().unwrap().display().to_string())
                );
            }
        ...
        Cmd::Bench {
            ...
            // W40: no explicit --transport = the tenant mode applies
            ...
            let t_kind = match transport {
                ...
                TransportKind::Auto => "auto",
                TransportKind::TcpFirst => "tcp-first",
            };
        ...
        Cmd::Access {
            ...
            // W4.1 [ADR-0016]: private-DNS mode — UDP+TCP responder (A-only,
            // no recursion) + zone-driven forwarders refreshed from the
            // controller's /v1/dns-zones (every 10 s). W43 S2: every spawn
            // here selects on the engine-generation token, so a
            // svc-disconnect frees the ports and stops the poller (detached
            // spawns survive a task abort — that was the leak).
            if let Some(spec) = &dns {
                ...
                std_sock.set_nonblocking(true)?;
                let sock = tokio::net::UdpSocket::from_std(std_sock)?;
                let tcp_l = tokio::net::TcpListener::bind((dhost.as_str(), dport)).await?;
                let zones: Arc<tokio::sync::RwLock<std::collections::HashMap<String, String>>> =
                    Arc::new(Default::default());
                let dns_cancel = svc::engine_cancel();
                println!("[dns] responder on {dhost}:{dport} udp+tcp (A-only, no recursion)");
                tokio::spawn(dns::serve(sock, zones.clone(), dns_cancel.clone()));
                tokio::spawn(dns::serve_tcp(tcp_l, zones.clone(), dns_cancel.clone()));
            ...
            // [DR-CTL-003]; URL configurable [P3-MG M5 — no hardcoded IPs])
            {
                ...
                tokio::spawn(async move {
                    ...
                    // W40 fail safe contract: warn once per DISTINCT bad
                    // mode value — a flapping bad value re-warns, a steady
                    // one does not spam (plan §3.1)
                    let mut warned_bad: Option<String> = None;
                    // W43 S2: engine-scoped (the posture-reporter twin)
                    let pv_cancel = svc::engine_cancel();
                    loop {
                        tokio::select! {
                            biased;
                            _ = pv_cancel.cancelled() => return,
                        ...
                        if let Ok(pv) = http().get(format!("{mgmt}/v1/policy-version")).send().await
                        {
                            if let Ok(j) = pv.json::<serde_json::Value>().await {
                                ...
                                last_pv = v;
                                // W40 [§3.4]: apply the mode from EVERY poll
                                // response — never gated on a version bump
                                // (a mode change bumps nothing in
                                // policy_version). Absent field (N-1
                                // controller) = auto per the fail-safe
                                // contract; unknown/malformed = auto + the
                                // parse-fallback event.
                                let (mode, bad) = aztna_common::TransportMode::from_json_fail_safe(
                                    j.get("transport_mode"),
                                );
                                if bad {
                                    let raw = j["transport_mode"].to_string();
                                    if warned_bad.as_deref() != Some(raw.as_str()) {
                                        println!(
                                            "[transport] policy-version carried unparseable transport_mode {raw} - auto [W40 fail-safe]"
                                        );
                                        log_event(
                                            "transport_mode_parse_fallback",
                                            &format!("value={raw} - auto"),
                                        );
                                        warned_bad = Some(raw);
                                    }
                                } else {
            ...
            // explicit-dest tasks, so park until the engine generation ends
            // (W43 S2: cancel-aware — the CLI parks forever, a service
            // disconnect returns; responder + zone manager own the lifetime)
            if dns.is_some() && dest.is_empty() {
                let park_cancel = svc::engine_cancel();
                loop {
                    tokio::select! {
                        biased;
                        _ = park_cancel.cancelled() => return Ok(()),
        ...
        Cmd::SplitdnsClean => {
            // W43 S3 [plan §11 rollback]: tag-filtered manual cleanup —
            // deletes only rules carrying the aztna:w43 ownership tag
            #[cfg(windows)]
            {
                let n = splitdns::watchdog::clean().await;
                println!("splitdns-clean: removed {n} owned rule(s)");
                if n == 0 {
                    println!("(no owned rules found — nothing to remove)");
                }
            }
            #[cfg(target_os = "macos")]
            {
                let n = splitdns::watchdog_macos::clean().await;
                println!("splitdns-clean: removed {n} owned resolver file(s)");
                if n == 0 {
                    println!("(no owned resolver files found — nothing to remove)");
                }
            }
            #[cfg(not(any(windows, target_os = "macos")))]
            println!("split-DNS auto mode is Windows/macOS-only; nothing to clean");
        }
    }
    Ok(())
}
...
async fn serve_dest(
    ...
    quic_ident: QuicIdent,
    relay_override: Option<String>,
    // W40: None = follow the tenant transport mode (resolved per dial so
    // a mode change applies to dials made after the poll that carried it)
    transport: Option<TransportKind>,
) -> anyhow::Result<()> {
    let Some((host, port_s)) = dest.split_once(':') else {
        anyhow::bail!("dest must be fqdn-or-ip:port");
    ...
    loop {
        ...
        for (gid, relay, tls_capable) in &candidates {
            // W40: resolve per dial (plan §2 — the mode applies to dials
            // made after the field is seen; already-active connections
            // are never dropped)
            let transport = resolve_transport(transport);
...
async fn zone_manager(
    ...
    quic_ident: QuicIdent,
    relay_override: Option<String>,
    // W40: None = follow the tenant transport mode (threaded to the
    // spawned serve_dest forwarders, resolved per dial there)
    transport: Option<TransportKind>,
    zones: Arc<tokio::sync::RwLock<std::collections::HashMap<String, String>>>,
) -> anyhow::Result<()> {
    // W43 S2: svc-disconnect cancels the engine generation — stop polling,
    // tear down forwarders, leave nothing of the namespace resident.
    // (Process-global token fetched at loop start, serve_dest's pattern.)
    let cancel = svc::engine_cancel();
    // W43 S3.2: system-wide split-DNS reconciler. This loop IS the health
    // lease (the heartbeat is written by the reconciler at the end of each
    // cycle) and the reconcile cadence (one tick per refresh). Config comes
    // from the service's own svc.toml — absent/inactive ⇒ inert (CLI --serve
    // lanes that never opted in are unaffected).
    let mut splitdns = {
        let cfg = svc::load_config().unwrap_or_default();
        let r = splitdns::reconciler::Reconciler::new(cfg.split_dns.clone());
        if r.active() {
            println!(
                "[splitdns] auto mode active (generation {}; allowlist: {} suffix(es))",
                r.generation(),
                cfg.split_dns.suffix_allowlist.len()
            );
        }
        r
    };
    // W43 S3.3: bring the cleanup owner up BEFORE any rule installs (the
    // watchdog is a precondition, not an assumption — plan §4.4). Failure ⇒
    // no_watchdog: auto mode refused, manual fallback.
    #[cfg(windows)]
    let session_objects = if splitdns.active() {
        let gen = splitdns.generation().to_string();
        match splitdns::watchdog::activate(&gen) {
            Some(objs) => {
                println!("[splitdns] watchdog active (L1 companion + L2 1-min/boot backstop task)");
                Some(objs)
            }
            None => {
                splitdns.deactivate();
                println!("[splitdns] watchdog NOT available (auto mode refused - manual fallback; see client.log)");
                log_event(
                    "splitdns_conflict",
                    "result=no_watchdog - auto mode refused (task registration or companion spawn failed), manual fallback",
                );
                metrics::splitdns_rule_ops()
                    .with_label_values(&["windows", "conflict", "no_watchdog"])
                    .inc();
                None
            }
        }
    ...
    // W43 S5.2: same precondition on macOS — the launchd backstop
    // (RunAtLoad + 30 s tick; no companion — launchd polls natively).
    // No session objects: graceful teardown removes our files via the
    // reconciler's shutdown, and the tick no-ops on a fresh heartbeat.
    #[cfg(target_os = "macos")]
    if splitdns.active() && !splitdns::watchdog_macos::activate() {
        splitdns.deactivate();
        println!("[splitdns] watchdog NOT available (auto mode refused - manual fallback; see client.log)");
        log_event(
            "splitdns_conflict",
            "result=no_watchdog - auto mode refused (launchd job registration failed - needs the root daemon context), manual fallback",
        );
        metrics::splitdns_rule_ops()
            .with_label_values(&["macos", "conflict", "no_watchdog"])
            .inc();
    }
    #[cfg(not(windows))]
    let session_objects = ();
    ...
    loop {
        match fetch_dns_zones(&mclient, &controller_url, &token).await {
            ZonesFetch::Ok(entries) => {
                ...
                // W43 S3.2: healthy cycle — reconcile rules to the fresh map
                let names: Vec<String> = entries
                    .iter()
                    .map(|z| z.fqdn.to_ascii_lowercase())
                    .collect();
                splitdns.tick(&names, true).await;
            }
            ZonesFetch::Denied(reason) => {
                ...
                // W43 S3.2/W6.3: the namespace shape must not stay resident —
                // desired set goes empty (still a healthy cycle: a definitive
                // server answer)
                splitdns.tick(&[], true).await;
            }
            ZonesFetch::Failed(e) => {
                println!("[dns] zone fetch failed: {e} - keeping current zones");
                // W43 S3.2: last-known serving — reconcile against the
                // retained map, but this cycle is NOT fully healthy for the
                // lease's hold-lift gate
                let names: Vec<String> = zones.read().await.keys().cloned().collect();
                splitdns.tick(&names, false).await;
            }
        }
        tokio::select! {
            biased;
            _ = cancel.cancelled() => {
                // W43 S2: session over — same hygiene as a denial (W6.3):
                // no forwarders, no name map resident. serve_dest tasks
                // self-exit via the token too; abort is belt-and-braces.
                let stopped = live.len();
                for (_, h) in live.drain() {
                    h.abort();
                }
                zones.write().await.clear();
                // W43 S3.2: graceful disconnect removes ALL owned rules,
                // THEN (S3.3) tell the companion to stand down — order
                // matters: the rules are already gone, so it exits without
                // touching anything
                splitdns.shutdown().await;
                #[cfg(windows)]
                if let Some(objs) = &session_objects {
                    objs.signal_standdown();
                }
                let _ = &session_objects;
                println!("[dns] engine generation cancelled - {stopped} forwarder(s) removed, zone map cleared");
                return Ok(());
            }
            _ = tokio::time::sleep(std::time::Duration::from_secs(10)) => {}
        }
...
enum RelayChannel {
    ...
    /// (SSL-strip guard, w14 plan). Boxed: the stream is ~1.1 kB and
    /// would otherwise pad every RelayChannel (incl. plain TCP) on the
...
async fn connect_relay(
    ...
    let dial_to = aztna_common::env_secs("AZTNA_QUIC_DIAL_TIMEOUT_SECS", 5, 1);
    // W40: carrier-selected ticks with the mode ACTUALLY APPLIED for this
    // dial (post-resolution, CLI override included)
    let selected = |carrier: &'static str| {
        metrics::carrier_selected(applied_mode_label(transport), carrier);
    };
    ...
    let plain = |reason: &'static str| async {
        ...
        match tokio::time::timeout(dial_to, tokio::net::TcpStream::connect(relay)).await {
            Ok(Ok(s)) => {
                selected("tcp");
                Ok::<_, anyhow::Error>(RelayChannel::Tcp(s))
            }
            Ok(Err(e)) => Err(e.into()),
            Err(_) => {
                metrics::tunnel_setup_timeouts()
    ...
    // W40 [tcp_first]: the TLS step's outcome is SPLIT so the carrier
    // preference can be inverted WITHOUT touching the eligibility
    // predicate — `auto`/`tls-tcp` answer Skip/Eligible with the W14
    // plain-TCP leg (unchanged semantics, still counted); `tcp_first`
    // answers the SAME classes with QUIC (the next mTLS carrier — plain
    // is never selected there). Fatal (handshake/cert failure, or
    // refused-despite-promise) fails the dial in EVERY mode
    // [SSL-strip guard — never a carrier switch].
    enum TlsStep {
        Connected(RelayChannel),
        /// capability says the carrier is absent — skip it entirely
        Skip(&'static str),
        /// refused + the W14 guard allows a fallback
        Eligible(&'static str),
        Fatal(anyhow::Error),
    }
    let tls_step = || async {
        if tls_capable == Some(false) || tls_tcp_known(relay) == Some(false) {
            println!("[transport] tls-tcp known-absent for {relay}");
            return TlsStep::Skip("capability_absent");
        }
        match tls_tcp_connect(host, port, ca_pem, ident, dial_to).await {
            ...
            Ok(None) => {
                if !refused_fallback_allowed(tls_capable) {
                    return TlsStep::Fatal(anyhow::anyhow!(
                        "tls-tcp refused although the controller promised the carrier - failing closed"
                    ));
                }
                remember_tls_tcp(relay, false);
                println!("[transport] tls-tcp refused (legacy gateway)");
                TlsStep::Eligible("refused_legacy")
            }
            Err(e) => TlsStep::Fatal(
    ...
    // the W14 answer to Skip/Eligible: the plaintext leg (counted)
    let plain_answer = |r: &'static str| async move {
        println!("[transport] - plain TCP to {relay} (counted downgrade: {r})");
        plain(r).await
    };
    // the W40 tcp_first answer to the SAME classes: the QUIC leg
    let quic_answer = || async {
        let c = quic_connect(host, port, ca_pem, ident, dial_to).await?;
        selected("quic");
        Ok::<_, anyhow::Error>(c)
    };
    match transport {
        TransportKind::Tcp => plain("legacy_flag").await,
        TransportKind::TlsTcp => match tls_step().await {
            TlsStep::Connected(c) => {
                selected("tls_tcp");
                Ok(c)
            }
            TlsStep::Skip(r) | TlsStep::Eligible(r) => plain_answer(r).await,
            TlsStep::Fatal(e) => Err(e),
        },
        TransportKind::Quic => quic_answer().await,
        ...
        // W40 [tcp_first, plan §3.5 — the normative state machine]: the
        // EXISTING auto fallback machinery with the carrier preference
        // inverted. Same fallback-ELIGIBILITY predicate (Skip/Eligible
        // are exactly the classes auto answers with plain), same timing
        // shape (the 1500 ms probe budget on the preferred carrier, then
        // the full dial on the second), reconnect backoff only after the
        // whole attempt fails, and no health-triggered re-probe while
        // connected. Plain TCP is NEVER selected; the cert-fail class
        // (Fatal) never switches carriers — it fails the dial.
        TransportKind::TcpFirst => {
            match tokio::time::timeout(std::time::Duration::from_millis(1500), tls_step()).await {
                Ok(TlsStep::Connected(c)) => {
                    selected("tls_tcp");
                    Ok(c)
                }
                Ok(TlsStep::Skip(r)) | Ok(TlsStep::Eligible(r)) => {
                    println!("[transport] tls-tcp {r} - trying QUIC (tcp_first)");
                    quic_answer().await
                }
                Err(_) => {
                    println!("[transport] tls-tcp probe timeout - trying QUIC (tcp_first)");
                    quic_answer().await
                }
                Ok(TlsStep::Fatal(e)) => Err(e),
            }
        }
    }
}
...
async fn tls_tcp_connect(
    ...
    Ok(Some(RelayChannel::TlsTcp(Box::new(stream))))
}
...
mod tests {
    ...
    /// Tests mutate the process-global tenant mode — one lockstep helper,
    /// always restored to Auto at the end (cargo runs tests in threads;
    /// the W40 tests are the only writers).
    fn w40_tenant(m: aztna_common::TransportMode) {
        set_tenant_mode(m);
    }

    #[test]
    fn w40_resolve_transport_precedence() {
        // explicit CLI choice always wins — the dev/test escape hatch
        w40_tenant(aztna_common::TransportMode::QuicOnly);
        assert!(matches!(
            resolve_transport(Some(TransportKind::Tcp)),
            TransportKind::Tcp
        ));
        assert!(matches!(
            resolve_transport(Some(TransportKind::Auto)),
            TransportKind::Auto
        ));
        // no explicit choice: each tenant mode maps to its carrier shape
        // (the legacy plaintext `tcp` kind is reachable ONLY via the CLI)
        w40_tenant(aztna_common::TransportMode::Auto);
        assert!(matches!(resolve_transport(None), TransportKind::Auto));
        w40_tenant(aztna_common::TransportMode::TcpFirst);
        assert!(matches!(resolve_transport(None), TransportKind::TcpFirst));
        w40_tenant(aztna_common::TransportMode::QuicOnly);
        assert!(matches!(resolve_transport(None), TransportKind::Quic));
        w40_tenant(aztna_common::TransportMode::TcpOnly);
        assert!(matches!(resolve_transport(None), TransportKind::TlsTcp));
        w40_tenant(aztna_common::TransportMode::Auto);
    }

    #[test]
    fn w40_applied_mode_labels() {
        assert_eq!(applied_mode_label(TransportKind::Tcp), "cli_legacy_tcp");
        assert_eq!(applied_mode_label(TransportKind::Quic), "quic_only");
        assert_eq!(applied_mode_label(TransportKind::TlsTcp), "tcp_only");
        assert_eq!(applied_mode_label(TransportKind::Auto), "auto");
        assert_eq!(applied_mode_label(TransportKind::TcpFirst), "tcp_first");
    }

    #[test]
    fn w40_state_transport_mode_lenient() {
        // a string value round-trips
        let st: ClientState =
            serde_json::from_str(r#"{"controller_url":"x","transport_mode":"tcp_first"}"#).unwrap();
        assert_eq!(st.transport_mode.as_deref(), Some("tcp_first"));
        // a non-string value degrades to None — never a state-load failure
        let st: ClientState =
            serde_json::from_str(r#"{"controller_url":"x","transport_mode":3}"#).unwrap();
        assert_eq!(st.transport_mode, None);
        // absent -> None (auto)
        let st: ClientState = serde_json::from_str(r#"{"controller_url":"x"}"#).unwrap();
        assert_eq!(st.transport_mode, None);
    }

    #[test]
    fn w40_poll_mode_decode() {
        // the poll's every-response decode rides the shared fail-safe fn:
        // valid -> the mode; absent -> auto, no fallback; unknown -> auto
        // WITH the fallback flag (drives the parse-fallback event)
        let j: serde_json::Value =
            serde_json::from_str(r#"{"version":7,"transport_mode":"quic_only"}"#).unwrap();
        let (m, bad) = aztna_common::TransportMode::from_json_fail_safe(j.get("transport_mode"));
        assert!(matches!(m, aztna_common::TransportMode::QuicOnly) && !bad);
        let j: serde_json::Value = serde_json::from_str(r#"{"version":7}"#).unwrap();
        let (m, bad) = aztna_common::TransportMode::from_json_fail_safe(j.get("transport_mode"));
        assert!(matches!(m, aztna_common::TransportMode::Auto) && !bad);
        let j: serde_json::Value =
            serde_json::from_str(r#"{"version":7,"transport_mode":"bogus"}"#).unwrap();
        let (m, bad) = aztna_common::TransportMode::from_json_fail_safe(j.get("transport_mode"));
        assert!(matches!(m, aztna_common::TransportMode::Auto) && bad);
        let j: serde_json::Value =
            serde_json::from_str(r#"{"version":7,"transport_mode":true}"#).unwrap();
        let (m, bad) = aztna_common::TransportMode::from_json_fail_safe(j.get("transport_mode"));
        assert!(matches!(m, aztna_common::TransportMode::Auto) && bad);
    }
    ...
    /// W37 [FR-BND-004 guard]: classification precedence — all four
    /// combinations pinned (docs/w37-enroll-guard-plan.md §4).
    #[test]
    fn enroll_guard_classifies_all_four_states() {
        let fresh = ClientState::default();
        assert_eq!(enrolled_state(&fresh), None);

        let mut pending = ClientState::default();
        pending.device_id = Some("dev-p".into());
        assert_eq!(
            enrolled_state(&pending),
            Some(EnrolledState::Pending {
                device_id: "dev-p".into()
            })
        );

        let mut approved = ClientState::default();
        approved.device_id = Some("dev-a".into());
        approved.cert_pem = Some("-----BEGIN CERTIFICATE-----".into());
        assert_eq!(
            enrolled_state(&approved),
            Some(EnrolledState::Approved {
                device_id: "dev-a".into()
            })
        );

        let mut cert_only = ClientState::default();
        cert_only.cert_pem = Some("-----BEGIN CERTIFICATE-----".into());
        assert_eq!(enrolled_state(&cert_only), Some(EnrolledState::CertOnly));
    }

    /// W37: the operator-facing refusal text is part of the feature —
    /// asserted, not prose. Pending: device_id + `pending` + the
    /// approval/login path + the state dir; must NOT suggest `renew`
    /// (renewal presents the CURRENT cert, which a pending device has
    /// not received) or imply a certificate exists.
    #[test]
    fn enroll_guard_pending_message_contract() {
        let es = EnrolledState::Pending {
            device_id: "dev-p".into(),
        };
        let msg = enroll_refusal_msg(&es, "C:/state");
        assert!(msg.contains("dev-p"), "names the device: {msg}");
        assert!(msg.contains("pending"), "classifies as pending: {msg}");
        assert!(
            msg.contains("C:/state"),
            "carries the state-dir path: {msg}"
        );
        assert!(msg.contains("login"), "points at the approval path: {msg}");
        assert!(
            msg.contains("no certificate"),
            "states the cert absence explicitly: {msg}"
        );
        assert!(
            !msg.contains("renew"),
            "must NOT suggest renew for a pending device: {msg}"
        );
    }

    /// W37: approved message contract — device_id + `approved` +
    /// `renew` (valid here: renewal presents the current cert) + the
    /// state-dir wipe path for a true re-enroll as a new device.
    #[test]
    fn enroll_guard_approved_message_contract() {
        let es = EnrolledState::Approved {
            device_id: "dev-a".into(),
        };
        let msg = enroll_refusal_msg(&es, "C:/state");
        assert!(msg.contains("dev-a"), "names the device: {msg}");
        assert!(msg.contains("approved"), "classifies as approved: {msg}");
        assert!(
            msg.contains("renew"),
            "offers the same-identity remedy: {msg}"
        );
        assert!(
            msg.contains("C:/state"),
            "carries the state-dir path: {msg}"
    ...
    /// W37: CertOnly gets ONLY the inconsistent-state naming + the wipe
    /// path — no classification language inherited from the other
    /// branches (the copy-paste-drift guard, review round 2).
    #[test]
    fn enroll_guard_cert_only_message_contract() {
        let msg = enroll_refusal_msg(&EnrolledState::CertOnly, "C:/state");
        assert!(
            msg.contains("inconsistent local state"),
            "names the inconsistent state: {msg}"
        );
        assert!(
            msg.contains("C:/state"),
            "carries the state-dir path: {msg}"
        );
        assert!(!msg.contains("renew"), "no renew advice: {msg}");
        assert!(!msg.contains("pending"), "no pending classification: {msg}");
        assert!(
            !msg.contains("approved"),
            "no approved classification: {msg}"
        );
    }
    ...
    fn cert_expiry_parsing_and_day_math() {
        ...
        let mut params = rcgen::CertificateParams::new(vec!["exp-test".into()]).unwrap();
        params
            .distinguished_name
