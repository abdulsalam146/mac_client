...
#[cfg(test)]
mod tests {
    // windows-only glob: the Windows test variants below call these
    // unqualified; the unix variants use full paths (so unix sees the
    // import as unused -> cfg-gated to keep both builds warning-free)
    #[cfg(windows)]
    use super::*;
