...
/// W43 S2 fix: ONE shared cell. The pre-fix twin `static T` declarations
/// (one per function) were two distinct OnceLocks — arm_fresh replaced a
/// token nobody read while engine_cancel() forever returned the token
/// cancelled by the FIRST disconnect, so a second connect could never
/// serve (latent since W13; found by the w43s2 reconnect check).
fn cancel_cell() -> &'static Mutex<tokio_util::sync::CancellationToken> {
...
pub fn engine_cancel() -> tokio_util::sync::CancellationToken {
    cancel_cell().lock().unwrap().clone()
}

pub fn engine_cancel_arm_fresh() {
    *cancel_cell().lock().unwrap() = tokio_util::sync::CancellationToken::new();
}
...
pub struct SvcConfig {
    ...
    /// W43 S3: system-wide split-DNS (plan `docs/w43-split-dns-plan.md`).
    /// Inert in S3.1 — the reconciler consumes it from S3.2; defaults per
    /// D4 (auto on Windows, manual elsewhere) and D10 (empty allowlist ⇒
    /// auto installs nothing).
    #[serde(default)]
    pub split_dns: crate::splitdns::SplitDnsConfig,
}
...
#[cfg(test)]
mod w43s3_parse_tests {
    use super::*;

    #[test]
    fn splitdns_table_parses() {
        let t = "controller_url = \"https://x\"\n\n[split_dns]\nmode = \"auto\"\nsuffix_allowlist = [\"corp\"]\n";
        let cfg: SvcConfig = toml::from_str(t).unwrap();
        assert_eq!(cfg.split_dns.mode, crate::splitdns::SplitDnsMode::Auto);
        assert_eq!(cfg.split_dns.suffix_allowlist, vec!["corp".to_string()]);
    }
}
